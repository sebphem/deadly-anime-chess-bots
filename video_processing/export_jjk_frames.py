...
def worker_function(episode_num):
    ...
    try:
        os.mkdir('%s/jjk_images/S1_(%s)' % (os.path.dirname(__file__),episode_num))
    except:
        pass
    jjk = cv2.VideoCapture('%s/jjk/Jujutsu Kaisen S1 (%s).mkv' % (os.path.dirname(__file__),episode_num))
    ...
    count = 0
    while success:
        cv2.imwrite("%s/jjk_images/S1_(%s)/frame%d.png"% (os.path.dirname(__file__),episode_num,count), frame)
        success , frame =jjk.read()
        count+= 1
